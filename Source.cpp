...
void levelUp(Object& player)
{
	...
	*/
	...
	player.health = randomHealth(engine);
	player.strength = randomStrength(engine);
	
	Item::Type chosenItem;
	chosenItem = (Item::Type)randomItem(engine);
	int itemsBonus = randomBonus(engine); 

	switch (chosenItem) {
	case Item::Type::sword:
		if (player.inventory.find(Item::Type::sword) != player.inventory.end()) {
			if (player.inventory.at(Item::Type::sword).bonusValue < itemsBonus) {
				player.inventory.push_back(found);
			}
		}
		else {
	...
	case Item::Type::armor:
		if (player.inventory.find(Item::Type::armor) != player.inventory.end()) {
			if (player.inventory.at(Item::Type::armor).bonusValue < itemsBonus) {
				player.inventory.push_back(found);
			}
		}
		else {
	...
	case Item::Type::shield:
		if (player.inventory.find(Item::Type::shield) != player.inventory.end()) {
			if (player.inventory.at(Item::Type::shield).bonusValue < itemsBonus) {
				player.inventory.push_back(found);
			}
		}
		else {
	...
	/*if (auto haveOne{ player.inventory.find(found.clasification) };
		haveOne == player.inventory.end() || player.inventory[found.classification].bonusValue < found.bonusValue) {
